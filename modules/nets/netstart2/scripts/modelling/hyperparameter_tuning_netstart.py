...
class MultiInputDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item = {
            ...
            },  # get both inputs and attention masks
...
class NetstartModel(nn.Module):
    ...
    def __init__(
        ...
        self.pretrained_model_aa = AutoModel.from_pretrained(
            "../../data/data_model/pretrained_models/finetuned_models/esm2-8m-finetuned_model_100u_100d_model"
            + str(self.model_no)
        )
...
def objective(trial, model_no):
    ...
    hidden_neurons_aa = trial.suggest_categorical(
        "hidden_neurons_aa", num_hidden_neurons
    )
    hidden_neurons_nt = trial.suggest_categorical(
        "hidden_neurons_nt", num_hidden_neurons
    )
    hidden_neurons_merge = trial.suggest_categorical(
        "hidden_neurons_merge", num_hidden_neurons
    )
    ...
    df_train, df_val = extract_datasets(
        data_train,
        data_val,
        tax_mapping,
    ...
    val_dataset = MultiInputDataset(
        val_encodings_aa, val_encodings_nt, df_val["tax_ranks_val"], labels_val
    )
    ...
    print(
        f"Memory Allocated before loading model: {torch.cuda.memory_allocated(device) / 1024**3} GB"
    )
    print(f"Running on: {device}")
    netstart_model.to(device)
    print(
        f"Memory Allocated after loading model: {torch.cuda.memory_allocated(device) / 1024**3} GB"
    )
    ...
    threshold_patience = (
        5  # Number of evaluation cycles with no improvement (based on validation loss)
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            if step % steps_between_evals == 0:
                print(
                    f"Memory Allocated start training: {torch.cuda.memory_allocated(device) / 1024**3} GB"
                )
            ...
            if step % steps_between_evals == 0:
                print(
                    f"Memory Allocated end training, before clear-up: {torch.cuda.memory_allocated(device) / 1024**3} GB"
                )
            ...
            if step % steps_between_evals == 0:
                print(
                    f"Memory Allocated end training, after clear-up: {torch.cuda.memory_allocated(device) / 1024**3} GB"
                )
            ...
            if step % steps_between_evals == 0:
                ...
                with torch.no_grad():
                    ...
                    print(
                        f"Memory Allocated end validation, before clear-up: {torch.cuda.memory_allocated(device) / 1024**3} GB"
                    )
                    ...
                    print(
                        f"Memory Allocated end validation, after clear-up: {torch.cuda.memory_allocated(device) / 1024**3} GB"
                    )

                    print(
                        f"ROC score: {roc_auc_score(val_true_labels, val_predicted_probs)}",
                        flush=True,
                    )
...
)  # Runs 30 trials to find the best hyperparameters
