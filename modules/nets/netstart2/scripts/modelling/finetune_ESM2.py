...
def extract_datasets(data_train, data_val, extract_upstream_aa, extract_downstream_aa):
    """
...
aa_sequences_train, labels_train, aa_sequences_val, labels_val = extract_datasets(
    data_train, data_val, extract_upstream_aa, extract_downstream_aa
)
...
print(
    "Fraction of TIS-labelled samples in training data: ",
    round(sum(labels_train) / len(labels_train), 3),
)
...
tokenizer_aa = AutoTokenizer.from_pretrained(
    esm_model, do_lower_case=False, model_max_length=aa_seqs_len + 2
)  # CLS token + aa sequence (1 token each) + EOS token
...
threshold_patience = (
    6  # Number of evaluations with no improvement to wait before stopping
)
counter_patience = 0
    ...
    for i, batch in enumerate(train_loader):
        ...
        if step % steps_between_evals == 0:
            ...
            with torch.no_grad():
                ...
                # Print metrics
                print(
                    f"Train Loss: {train_avg_loss}\tVal Loss: {val_avg_loss}\tVal AUC ROC: {val_auc_roc}\tVal AUC PR: {val_pr_auc}",
                    sep="",
                    flush=True,
                )
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    base_model.save_pretrained(
                        ...
                        + esm_model_title
                        + "-finetuned_model_100u_100d_model"
                        + str(model_no)
                    )
                    ...
                    model.save_pretrained(
                        ...
                        + esm_model_title
                        + "_finetuned_full_model_100u_100d_model"
                        + str(model_no)
                        + ".pth"
                    )
