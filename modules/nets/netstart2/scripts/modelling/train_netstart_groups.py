...
def extract_datasets(
    ...
    for i, row in enumerate(data_train.itertuples()):
        ...
        assert row.Sequence[ATG_position : ATG_position + 3] == "ATG", (
            f"Expected 'ATG' at position {ATG_position}, but found {row.Sequence[ATG_position : ATG_position + 3]}"
        )
...
def create_encodings(
    df_train, df_val, df_test, extract_upstream_aa, extract_downstream_aa, tokenizer_aa
):
    """
...
def main(org_group, model_no):
    # Load Data
    data_train, data_val, data_test = load_and_partition_data(
        model_no=model_no, org_group=org_group
    )
    ...
    checkpoint = torch.load(
        "../../data/data_model/models/netstart_model" + str(model_no) + ".pth",
        ...
        for batch in train_loader:
            ...
            if step % steps_between_evals == 0:
                ...
                with torch.no_grad():
                    ...
                    val_auc_roc = round(
                        roc_auc_score(val_true_labels, val_predicted_probs), 4
                    )
                    ...
                    if val_avg_loss < best_val_loss:
                        ...
                        if isinstance(
                            ...
                            netstart_model, torch.nn.parallel.DistributedDataParallel
                        ):
                            state_dict = (
    ...
    assert total == TP + FN + TN + FP, (
        "Something went wrong calculating test performance"
    )
