...
with open(
    "../../data/data_model/hyperparameters/" + hyperpars_filename, "r"
) as json_file:
...
def load_and_partition_data(model_no):
    ...
    data_partition_1 = pd.read_csv(
        "../../data/data_model/datasets/data_partition_1_masked.csv.gz",
        dtype={"annotation_source": "str"},
        compression="gzip",
    )
    data_partition_2 = pd.read_csv(
        "../../data/data_model/datasets/data_partition_2_masked.csv.gz",
        dtype={"annotation_source": "str"},
        compression="gzip",
    )
    data_partition_3 = pd.read_csv(
        "../../data/data_model/datasets/data_partition_3_masked.csv.gz",
        dtype={"annotation_source": "str"},
        compression="gzip",
    )
    data_partition_4 = pd.read_csv(
        "../../data/data_model/datasets/data_partition_4_masked.csv.gz",
        dtype={"annotation_source": "str"},
        compression="gzip",
    )
    data_partition_5 = pd.read_csv(
        "../../data/data_model/datasets/data_partition_5_masked.csv.gz",
        dtype={"annotation_source": "str"},
        compression="gzip",
    )
    ...
    if model_no == 1:
        ...
        data_train = pd.concat(
            [data_partition_2, data_partition_3, data_partition_4], ignore_index=True
        )
        ...
        # Concat train partitions
        data_train = pd.concat(
            [data_partition_1, data_partition_3, data_partition_4], ignore_index=True
        )
        ...
        # Concat train partitions
        data_train = pd.concat(
            [data_partition_1, data_partition_2, data_partition_4], ignore_index=True
        )
        ...
        # Concat train partitions
        data_train = pd.concat(
            [data_partition_1, data_partition_2, data_partition_3], ignore_index=True
        )
    ...
    data_train["Species"] = data_train["Species"].replace(
        "Canis lupus familiaris", "Canis lupus"
    )
    data_val["Species"] = data_val["Species"].replace(
        "Canis lupus familiaris", "Canis lupus"
    )
    data_test["Species"] = data_test["Species"].replace(
        "Canis lupus familiaris", "Canis lupus"
    )
...
def load_taxonomy_mappings():
    ...
    species_id_mapping = {
        ncbi_id: str(i + 1) for i, ncbi_id in enumerate(sorted(all_ids))
    }
    ...
    taxonomic_dicts = [
        ...
        species_class_dict,
        species_phylum_dict,
        species_kingdom_dict,
    ]

    taxonomic_id_mappings = [
        {ncbi_id: str(i + 1) for i, ncbi_id in enumerate(sorted(set(d.values())))}
        for d in taxonomic_dicts
    ]
...
def extract_datasets(
    ...
    for i, row in enumerate(data_train.itertuples()):
        ...
        assert row.Sequence[ATG_position : ATG_position + 3] == "ATG", print(
            row.Sequence[ATG_position : ATG_position + 3]
        )

        # Append each row of wanted information to the list
        rows.append(
            {
                "aa_sequences_train": encode_nucleotide_to_amino_acid(
                    row.Sequence[
                        ATG_position - 3 * extract_upstream_aa : ATG_position
                        + 3
                        + 3 * extract_downstream_aa
                    ]
                ),
                "nt_sequences_train": row.Sequence[
                    ATG_position - extract_upstream_nt : ATG_position
                    + 3
                    + extract_downstream_nt
                ],
    ...
    for i, row in enumerate(data_val.itertuples()):
        ...
        rows.append(
            {
                "aa_sequences_val": encode_nucleotide_to_amino_acid(
                    row.Sequence[
                        ATG_position - 3 * extract_upstream_aa : ATG_position
                        + 3
                        + 3 * extract_downstream_aa
    ...
    for i, row in enumerate(data_test.itertuples()):
        ...
        rows.append(
            {
                "aa_sequences_test": encode_nucleotide_to_amino_acid(
                    row.Sequence[
                        ATG_position - 3 * extract_upstream_aa : ATG_position
                        + 3
                        + 3 * extract_downstream_aa
                ...
                "nt_sequences_test": row.Sequence[
                    ATG_position - extract_upstream_nt : ATG_position
                    + 3
                    + extract_downstream_nt
                ],
...
def create_encodings(
    df_train, df_val, df_test, extract_upstream_aa, extract_downstream_aa
):
    """
    ...
    train_encodings_aa = tokenizer_aa(
        list(df_train["aa_sequences_train"]),
    ...
    train_encodings_aa["attention_mask"] = (
        attention_mask_train.int()
    )  # Replace the old attention mask

    val_encodings_aa = tokenizer_aa(
        list(df_val["aa_sequences_val"]),
        padding=True,  # pad sequences to max length and apply attention mask
        truncation=True,
        return_tensors="pt",
    )
    attention_mask_val = val_encodings_aa["input_ids"] != tokenizer_aa.pad_token_id
    val_encodings_aa["attention_mask"] = (
        attention_mask_val.int()
    )  # Replace the old attention mask

    test_encodings_aa = tokenizer_aa(
        list(df_test["aa_sequences_test"]),
        padding=True,  # pad sequences to max length and apply attention mask
        truncation=True,
        return_tensors="pt",
    )
    attention_mask_test = test_encodings_aa["input_ids"] != tokenizer_aa.pad_token_id
    test_encodings_aa["attention_mask"] = (
        attention_mask_test.int()
    )  # Replace the old attention mask
...
class MultiInputDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tax_ranks_item = [
            int(rank) if rank != "0" else 0 for rank in self.tax_ranks[idx]
        ]

        item = {
            "aa_encodings": {
                key: torch.as_tensor(val[idx]) for key, val in self.aa_encodings.items()
            },
            "nt_encodings": torch.as_tensor(
                self.nt_encodings[idx], dtype=torch.float32
            ),
...
class NetstartModel(nn.Module):
    ...
    def __init__(
        ...
        self.pretrained_model_aa = AutoModel.from_pretrained(
            "../../data/data_model/pretrained_models/finetuned_models/"
            + esm2_pretrained
        )
        ...
        self.hidden_layer_aa = nn.Linear(
            self.pretrained_model_aa.config.hidden_size * aa_encoding_length,
            hidden_neurons_aa,
        )

        # Define feedforward hidden layers for local start codon context window
        self.nt_layers = nn.ModuleList()
        self.nt_layers.append(
            nn.Linear(nt_encoding_length, hidden_neurons_nt)
        )  # First layer
        ...
        self.hidden_layer_2 = nn.Linear(
            hidden_neurons_aa + hidden_neurons_nt + emb_size, hidden_neurons_merge
        )
    ...
    def forward(self, x_aa, x_nt, attention_mask_aa, tax_ranks):
        """
        ...
        x_nt = x_nt.view(
            x_nt.size(0), -1
        )  # Reshape the input to flatten it (4x23 to 1D tensor)
...
# Get data
df_train, df_val, df_test = extract_datasets(
    data_train,
    data_val,
    data_test,
    tax_mapping,
...
nt_encodings_len_flattened = len(train_encodings_nt[0]) * len(
    train_encodings_nt[0][0]
)  # 4 * numbers of nt
...
train_dataset = MultiInputDataset(
    train_encodings_aa,
    train_encodings_nt,
    df_train["tax_ranks_train"],
    df_train["group_train"],
    labels_train,
)
val_dataset = MultiInputDataset(
    val_encodings_aa,
    val_encodings_nt,
    df_val["tax_ranks_val"],
    df_val["group_val"],
    labels_val,
)
...
if torch.cuda.is_available() and torch.cuda.device_count() > 1:
    ...
    netstart_model = torch.nn.DataParallel(
        netstart_model
    )  # This wraps your model to use multiple GPUs
...
steps_per_epoch = len(train_dataset) / (
    batch_size
)  # The number of steps taken per epoch
eval_per_epoch = 16  # The number of evaluations made per epoch
steps_between_evals = int(
    steps_per_epoch / eval_per_epoch
)  # The number of steps to take between each evaluation
total_eval_steps = (
    eval_per_epoch * num_epochs
)  # The total number of times the model is evaluated during finetuning

print(
    f"Running for {num_epochs} epochs with a batchsize of {batch_size}. Total number of training sequences: {len(train_dataset)}.",
    sep="",
    flush=True,
)
...
threshold_patience = (
    12  # Number of evaluations with no improvement to wait before stopping
)
counter_patience = 0
...
wandb.init(
    ...
    config={"model_number": str(model_no)},
)
...
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        if step % int(steps_between_evals / 4) == 0:
            print(
                f"Step {step} / {int(steps_per_epoch) * num_epochs} (Total steps: {int(steps_per_epoch) * num_epochs})",
                sep=" ",
                flush=True,
            )
        ...
        if step % steps_between_evals == 0:
            ...
            with torch.no_grad():
                for count, batch in enumerate(val_loader):
                    ...
                    inputs_aa = batch["aa_encodings"]["input_ids"].to(device)
                    inputs_nt = batch["nt_encodings"].to(device)
                    attention_mask_aa = batch["aa_encodings"]["attention_mask"].to(
                        device
                    )
                    tax_ranks = batch["tax_ranks"].to(device)
                    ...
                    # Forward pass
                    outputs = netstart_model(
                        inputs_aa, inputs_nt, attention_mask_aa, tax_ranks
                    )
                    loss = loss_fn(outputs, labels)
                    ...
                    for i, g in enumerate(group):
                        ...
                        groupwise_predicted_probs[g].append(
                            torch.sigmoid(outputs[i]).item()
                        )
                ...
                val_auc_roc = round(
                    roc_auc_score(val_true_labels, val_predicted_probs), 6
                )
                precision, recall, _ = precision_recall_curve(
                    val_true_labels, val_predicted_probs
                )
                ...
                print(
                    f"Train Loss: {train_avg_loss}\tVal Loss: {val_avg_loss}\t"
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    if isinstance(
                        ...
                        state_dict = (
                            netstart_model.module.state_dict()
                    ...
                    torch.save(
                        state_dict,
                        "../../data/data_model/models/"
                        + netstart_model_filename
                        + ".pth",
                    )
                else:
                    counter_patience += 1
                ...
                if counter_patience >= threshold_patience:
                    print(
                        "Early stopping. No improvement in validation loss.", flush=True
                    )
                    break
...
for threshold in thresholds:
    mccs.append(
        matthews_corrcoef(
            ...
            (np.array(test_predicted_probs) >= threshold).astype(int),
        )
    )
...
TP = sum(
    (test_true_labels == 1.0) & (test_predicted_probs >= best_threshold)
    for test_true_labels, test_predicted_probs in zip(
        test_true_labels, test_predicted_probs
    )
)
FN = sum(
    (test_true_labels == 1.0) & (test_predicted_probs < best_threshold)
...
TN = sum(
    (test_true_labels == 0.0) & (test_predicted_probs < best_threshold)
    for test_true_labels, test_predicted_probs in zip(
        test_true_labels, test_predicted_probs
    )
)
...
mcc = matthews_corrcoef(
    np.array(test_true_labels),
    (np.array(test_predicted_probs) >= best_threshold).astype(int),
)
...
print(
    f"Test Sensitivity: {sensitivity:.4f}, \nTest Specificity: {specificity:.4f}\nTest MCC: {mcc:.4f}",
    flush=True,
)
