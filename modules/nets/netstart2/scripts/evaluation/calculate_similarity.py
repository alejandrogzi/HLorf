...
def align_sequences_at_tis(seq1, tis_pos1, seq2, tis_pos2):
    ...
    aligned_tis_pos = (
        upstream_min  # The TIS position for both sequences after alignment
    )
...
with h5py.File(
    "../../data/data_evaluation/TIS_transformer/dataset/GRCh38_v107.h5", "r"
) as h5_file:
    ...
    group = h5_file[
        "transcript"
    ]  # Replace 'group_name' with an actual group in your file
    ...
    for i in range(len(tis_labels)):
        ...
        if len(tis_pos) > 0:
            ...
            if (
                seq[i][tis_pos] == 0
                and seq[i][tis_pos + 1] == 1
                and seq[i][tis_pos + 2] == 3
            ):
                ...
                transcript_seqs_TIS_transformer[ids[i]]["tis_pos"] = tis_pos[
                    0
                ]  # Store the first TIS position
...
def process_species(species):
    ...
    for transcript_id_testset, data in transcript_seqs_testset.items():
        ...
        for transcript_id, data in transcript_seqs_TIS_transformer.items():
            ...
            aligned_seq_testset, aligned_seq_TIS_transformer, aligned_tis_pos = (
                align_sequences_at_tis(
                    sequence_testset,
                    tis_pos_testset,
                    sequence_TIS_transformer,
                    tis_pos_TIS_transformer,
                )
            )

            assert len(aligned_seq_testset) == len(aligned_seq_TIS_transformer)
            assert (
                aligned_seq_testset[aligned_tis_pos : aligned_tis_pos + 3]
                == aligned_seq_TIS_transformer[aligned_tis_pos : aligned_tis_pos + 3]
                == "ATG"
            )

            seq_identity = calculate_identity(
                aligned_seq_testset, aligned_seq_TIS_transformer
            )
