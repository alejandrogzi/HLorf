...
def extract_datasets(data_test, extract_upstream_aa, extract_downstream_aa):
    """
    ...
    for row in data_test.itertuples():
        ...
        aa_sequences_test.append(
            encode_nucleotide_to_amino_acid(
                row.Sequence[
...
class MultiInputDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        item = {
            "encodings": {
                key: torch.as_tensor(val[idx]) for key, val in self.encodings.items()
            },
...
def run_pca(model_name, tokenizer_name, finetuned):
    ...
    aa_sequences_test, labels_test, seq_type_test, species = extract_datasets(
        data_test, extract_upstream_aa=100, extract_downstream_aa=100
    )
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        tokenizer_name, do_lower_case=False, model_max_length=aa_seqs_len + 1
    )

    test_encodings = tokenizer(
        aa_sequences_test, padding=True, truncation=True, return_tensors="pt"
    )
    ...
    test_dataset = MultiInputDataset(
        test_encodings, labels_test, seq_type_test, species
    )
    ...
    pca_label_0 = [
        row for idx, row in enumerate(embeddings_pca) if labels_list[idx] == 0.0
    ]
    pca_label_1 = [
        row for idx, row in enumerate(embeddings_pca) if labels_list[idx] == 1.0
    ]
    ...
    plt.xlabel(
        "PC1 (" + str(round(variance_explained[0] * 100, 2)) + "% variance explained)",
        fontsize=15,
    )
    plt.ylabel(
        "PC2 (" + str(round(variance_explained[1] * 100, 2)) + "% variance explained)",
        fontsize=15,
    )
    ...
    plt.savefig(
        "../../../results/learned_model_embeddings/esm2_embeddings/PCA_binary_"
        + model_name_clean
        + ".png"
    )
    ...
    pca_upstream_out = [
        row
        for idx, row in enumerate(embeddings_pca)
        if seq_types_list[idx] == "Upstream, out of frame"
    ]
    pca_upstream_in = [
        row
        for idx, row in enumerate(embeddings_pca)
        if seq_types_list[idx] == "Upstream, in frame"
    ]
    pca_downstream_out = [
        row
        for idx, row in enumerate(embeddings_pca)
        if seq_types_list[idx] == "Downstream, out of frame"
    ]
    pca_downstream_in = [
        row
        for idx, row in enumerate(embeddings_pca)
        if seq_types_list[idx] == "Downstream, in frame"
    ]
    pca_intergenic = [
        row
        for idx, row in enumerate(embeddings_pca)
        if seq_types_list[idx] == "Intergenic"
    ]
    pca_introns = [
        row for idx, row in enumerate(embeddings_pca) if seq_types_list[idx] == "Intron"
    ]
    pca_TIS = [
        row for idx, row in enumerate(embeddings_pca) if seq_types_list[idx] == "TIS"
    ]
    ...
    plt.scatter(
        pca_upstream_out_PC1, pca_upstream_out_PC2, color=colors[3], alpha=0.8, s=18
    )
    plt.scatter(
        pca_upstream_in_PC1, pca_upstream_in_PC2, color=colors[1], alpha=0.7, s=18
    )
    plt.scatter(
        pca_downstream_out_PC1, pca_downstream_out_PC2, color=colors[9], alpha=0.7, s=18
    )
    plt.scatter(
        pca_downstream_in_PC1, pca_downstream_in_PC2, color=colors[6], alpha=0.7, s=18
    )
    ...
    #                'TIS'])
    plt.savefig(
        "../../../results/learned_model_embeddings/esm2_embeddings/PCA_seq_types_"
        + model_name_clean
        + ".png"
    )
...
data_test = pd.read_csv(
    "../../../data/data_model/datasets/data_partition_5.csv.gz", compression="gzip"
)
...
run_pca(
    model_name="../../../data/data_model/pretrained_models/finetuned_models/esm2-8m-finetuned_model_100u_100d_model1",
    tokenizer_name="facebook/esm2_t6_8M_UR50D",
    finetuned=True,
)

run_pca(
    model_name="../../../data/data_model/pretrained_models/finetuned_models/esm2-8m-finetuned_model_100u_100d_model2",
    tokenizer_name="facebook/esm2_t6_8M_UR50D",
    finetuned=True,
)

run_pca(
    model_name="../../../data/data_model/pretrained_models/finetuned_models/esm2-8m-finetuned_model_100u_100d_model3",
    tokenizer_name="facebook/esm2_t6_8M_UR50D",
    finetuned=True,
)

run_pca(
    model_name="../../../data/data_model/pretrained_models/finetuned_models/esm2-8m-finetuned_model_100u_100d_model4",
    tokenizer_name="facebook/esm2_t6_8M_UR50D",
    finetuned=True,
)
