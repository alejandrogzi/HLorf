...
class GFFProcesser:
    ...
    def extract_correct_positions(self, transcript_id):
        ...
        if transcript_id != "":
            ...
            # No annotated exons (no defined mature mRNA)
                ...
                flattened_exon_positions = [
                    item
                    for sublist in self.transcripts_info_dict[transcript_id]["exon_pos"]
                    for item in sublist
                ]
                min_exon = min(
                    flattened_exon_positions
                )  # beginning position of first exon
                ...
                if self.transcripts_info_dict[transcript_id]["mRNA_pos"][0] != min_exon:
                    ...
                    self.transcripts_info_dict[transcript_id]["mRNA_pos"][1] != max_exon
                ):
                    del self.transcripts_info_dict[transcript_id]
    ...
    def process_gff_file(self):
        ...
        with gzip.open(
            f"../../../data/data_raw/genomic_gff/{self.species}_genomic.gff.gz", "rt"
        ) as info_file:
                        ...
                        if col_info[2] == "mRNA":
                            ...
                            if match_id:
                                match_partial = re.search(
                                    r"partial=true", attr_col
                                )  # Look for partial annotations
                                ...
                                # Save information with complete annotations
                                    ...
                                    self.transcripts_info_dict[transcript_id] = {
                                        "mRNA_pos": [
                                            int(col_info[3]),
                                            int(col_info[4]),
                                        ],  # mRNA coordinates on chromosome
                                        ...
                                        "source": col_info[1],
                                    }  # Algorithm used for annotated feature
        ...
        print(
            "Number of transcripts extracted:", len(self.transcripts_info_dict.keys())
        )

        # Save dict with annotation information
        with open(
            f"../../../data/data_analysis/data_preparatory_analysis/transcripts_info/{self.species}_dict.json",
            "w",
        ) as dict_file:
...
class ExtractSequenceInformation:
        ...
        if annotations_dict[transcript_id]["Strand"] == "+":
            ...
            for exon_pos in sorted_exons:
                if exon_pos[0] <= CDS_start <= exon_pos[1]:
                    ...
                    TIS = (
                        exons_length + CDS_start - exon_pos[0]
                    )  # TIS: 0-index (starts in 0); corresponds to position on exons (mature mRNA)
                ...
                exon_seq += chromosome_seq[
                    exon_pos[0] - 1 : exon_pos[1]
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "Source": [annotations_dict[transcript_id]["source"]],
                            }
                        ),
                    ],  # Annotation source (BestRefseq, Gnomon, RefSeq)
                    ignore_index=True,
                )
        ...
        # Handle cases on complement strand
            ...
            for exon_pos in sorted_exons:
                if exon_pos[0] <= CDS_start <= exon_pos[1]:
                    # Locate TIS position in the mature mRNA (introns removed)
                    TIS = (
                        exons_length + exon_pos[1] - CDS_start
                    )  # TIS; 0-index (starts in 0); corresponds to position on exons (mature mRNA)
    ...
    def process_and_extract_sequence_info(self):
        ...
        TIS_information = pd.DataFrame(
            columns=["Transcript_id", "TIS_codon", "UTR_5_length", "Source"]
        )
        ...
        with gzip.open(
                    ...
                    if chromosome_seq_initialized:
                        ...
                        if extract_info:
                            ...
                            for i in range(len(transcripts_in_chromosome)):
                                TIS_information, cds_ends_correct, cds_ends_wrong = (
            ...
            TIS_information.to_csv(
                f"../../../data/data_analysis/data_preparatory_analysis/transcripts_data/transcript_data_{self.species}.csv",
                index=False,
            )
...
def main():
    ...
    species_filenames_list = os.listdir(
        "../../../data/data_evaluation/input_testsets/genomic_testsets"
    )
    species_list = [
        filename.split(".fasta.gz")[0].split("genomic_testset_")[1]
