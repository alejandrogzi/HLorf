...
with open(
    "../../data/data_model_ablations/hyperparameters/" + hyperpars_filename, "r"
) as json_file:
...
netstart1_model = NetStart1Model(
    nt_encoding_length=nt_encodings_len_flattened, vocab_sizes=vocab_sizes
)
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    if np.any(np.isnan(torch.sigmoid(outputs).cpu().numpy().flatten())):
                        ...
                        print(
                            torch.sigmoid(outputs).cpu().numpy().flatten(), flush=True
                        )
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    if isinstance(netstart1_model, torch.nn.DataParallel) or isinstance(
                        netstart1_model, torch.nn.parallel.DistributedDataParallel
                    ):
                        state_dict = (
                            netstart1_model.module.state_dict()
