...
def extract_datasets(
    data_train, data_val, data_test, extract_upstream_aa, extract_downstream_aa
):
    """
...
class ESM2Model(nn.Module):
    ...
    def __init__(self, aa_encoding_length, num_labels=1):
        ...
        self.classifier = nn.Linear(
            self.pretrained_model_aa.config.hidden_size * aa_encoding_length, num_labels
        )
...
train_dataset = MultiInputDataset(
    train_encodings_aa, df_train["group_train"], labels_train
)
        ...
        if step % steps_between_evals == 0:
            ...
            with torch.no_grad():
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    if isinstance(esm2_model, torch.nn.DataParallel) or isinstance(
                        esm2_model, torch.nn.parallel.DistributedDataParallel
                    ):
                        state_dict = (
                            esm2_model.module.state_dict()
                    ...
                    torch.save(
                        state_dict,
                        "../../data/data_model_ablations/models/"
                        + esm2_model_filename
                        + ".pth",
                    )
                else:
...
TP = sum(
    (test_true_labels == 1.0) and (test_predicted_probs >= best_threshold)
...
TN = sum(
    (test_true_labels == 0.0) and (test_predicted_probs < best_threshold)
    for test_true_labels, test_predicted_probs in zip(
        test_true_labels, test_predicted_probs
    )
)
