...
def extract_datasets(
    data_train, data_val, tax_mapping, extract_upstream_nt, extract_downstream_nt
):
    """
...
def create_encodings(df_train, df_val):
    ...
    return labels_train, sequences_train_nt, labels_val, sequences_val_nt
...
class NetStart1Model(nn.Module):
    ...
    def __init__(
        ...
        self.hidden_layer_2 = nn.Linear(
            hidden_neurons_nt + emb_size, hidden_neurons_merge
        )
    ...
    def forward(self, x_nt, tax_ranks):
...
def objective(trial):
    ...
    print(
        f"hidden_neurons_nt: {hidden_neurons_nt}\nhidden_neurons_merge: {hidden_neurons_merge}\ndepth_nt_window: {num_hidden_layers_nt}"
    )

    wandb.init(
        project=f"hyperparameter_optimization_netstart1_{str(model_no)}",
        config={
    ...
    labels_train, train_encodings_nt, labels_val, val_encodings_nt = create_encodings(
        df_train, df_val
    )
    ...
    train_dataset = MultiInputDataset(
        train_encodings_nt, df_train["tax_ranks_train"], labels_train
    )
    val_dataset = MultiInputDataset(
        val_encodings_nt, df_val["tax_ranks_val"], labels_val
    )
    ...
    # Instantiate model
    netstart1_model = NetStart1Model(
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
        ...
        netstart1_model = torch.nn.DataParallel(
            netstart1_model
    ...
    threshold_patience = (
        8  # Number of evaluation cycles with no improvement (based on validation loss)
    )
...
for model_no in [model_no]:
    ...
    file_path = (
        "../../data/data_model_ablations/hyperparameters/netstart1_model"
        + str(model_no)
