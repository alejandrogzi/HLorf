...
class ExtractSequences:
    ...
    def load_annotation_info_dict(self):
        ...
        try:
            with open(
                f"../../data/data_model_preparation/transcripts_info/{self.species}_dict.json",
                "r",
            ) as dict_file:
                annotations_dict = json.load(dict_file)

                print(
                    "Transcripts from annotation file:",
            ...
            print(
                f"Error: The annotation file for {self.species} was not found.",
                flush=True,
            )
            ...
            print(
                f"An unexpected error occurred for {self.species}: {error}", flush=True
            )
    ...
    def get_TIS_positions_gene(self):
        ...
        with open(
            f"../../data/data_model_preparation/transcripts_info/"
            + self.species
            + "_dict.json",
            "r",
        ) as dict_file:
            ...
            for key, value in annotations_dict.items():
                ...
                # Complement strand
                    ...
                    TIS = CDS[
                        ...
                        -1
                    ]  # TIS is end coordinate on first CDS (highest position, complement strand)
    ...
    def quality_check_extracted_sequences(
        self,
        TIS,
        CDS_end,
        exon_seq,
        ...
        if write_transcript_annotation:
            ...
            for i in range(codons_in_seq):
                if exon_seq[TIS + i * 3 : TIS + i * 3 + 3].lower() in [
                    "tga",
                    "tag",
                    "taa",
                ]:
            ...
            if strand == "+":
                if (
                    chromosome_seq[genomic_ATG_coord : genomic_ATG_coord + 3].upper()
                    != "ATG"
                ):  # ATG (genomic coordinates)
                    write_transcript_annotation = False
                ...
                if (
                    chromosome_seq[genomic_ATG_coord : genomic_ATG_coord + 3].upper()
                    != "CAT"
                ):  # reverse-complementet ATG (corresponds to genomic coordinates)
                    write_transcript_annotation = False
                    genomic_ATG_coord_false += 1
                else:
                    genomic_ATG_coord_correct += 1
    ...
    def extract_non_TIS_samples(
        ...
        try:
            ...
            atg_indices = [
                i for i in range(len(exon_seq)) if exon_seq.upper().startswith("ATG", i)
            ]
            ...
            indices_upstream_TIS = [
                index
                for index in atg_indices
                if index < TIS and index > self.nts_upstream_extract
            ]
            ...
            if len_5_UTR is not None:
                for i in range(len(indices_upstream_TIS)):
                    ...
                    assert (
                        exon_seq[
                            indices_upstream_TIS[i] : indices_upstream_TIS[i] + 3
                        ].upper()
                        == "ATG"
                    ), "ATG-label not extracted properly."

                    # Write correctly-labeled/true datapoint to dataframe
                    non_TIS_information = pd.concat(
                        [
                            non_TIS_information,
                            pd.DataFrame(
                                {
                                    "Sequence": exon_seq.upper()[
                                        indices_upstream_TIS[i]
                                        - self.nts_upstream_extract : indices_upstream_TIS[
                                            i
                                        ]
                                        + 3
                                        + self.nts_downstream_extract
                                    ],  # Extracted sequence
                                    ...
                                    "annotation_source": [
                                        annotations_dict[transcript_id]["source"]
                                    ],  # Source of annotation (Gnomon, RefSeq, BestRefSeq)
                                    ...
                                    "landmark_id": [
                                        annotations_dict[transcript_id]["chrom"]
                                    ],  # Chromosome
                                    "mrna_pos": [
                                        annotations_dict[transcript_id]["mRNA_pos"]
                                    ],  # The transcript coordinates
                                    "transcript_id": transcript_id,  # transcript id
                                    "strand": [
                                        annotations_dict[transcript_id]["Strand"]
                                    ],  # transcript strand
                                    "ATG_genomic_position": None,  # The TIS position (from genomic data)
                                    "gene": [
                                        annotations_dict[transcript_id]["gene"]
                                    ],  # gene name
                                    "gene_coordinates": [
                                        annotations_dict[transcript_id][
                                            "gene_coordinates"
                                        ]
                                    ],
                                }
            ...
            for atg_index in indices_downstream_TIS:
                ...
                if in_frame_search:
                    if (atg_index - TIS) % 3 == 0:
                        ...
                        if (
                            atg_index > self.nts_upstream_extract
                            and len(exon_seq) - (atg_index + 3)
                            > self.nts_downstream_extract
                        ):
                                    ...
                                    pd.DataFrame(
                                        {
                                            "Sequence": exon_seq.upper()[
                                                atg_index
                                                - self.nts_upstream_extract : atg_index
                                                + 3
                                                + self.nts_downstream_extract
                                            ],  # Extracted sequence
                                            "ATG_position": self.nts_upstream_extract,  # Position of ATG to predict on
    ...
    def extract_strand_information(
        self,
        annotations_dict,
        transcript_id,
        chromosome_seq,
        ...
        if annotations_dict[transcript_id]["Strand"] == "+":
            ...
            sorted_exons = sorted(
                annotations_dict[transcript_id]["exon_pos"], key=lambda x: x[0]
            )
            sorted_CDS = sorted(
                annotations_dict[transcript_id]["CDS_pos"], key=lambda x: x[0]
            )
            ...
            for exon_pos in sorted_exons:
                ...
                if exon_pos[0] <= CDS_stop <= exon_pos[1]:
                    ...
                    CDS_end = (
                        exons_length + CDS_stop - exon_pos[0] + 1
                    )  # CDS_end: 0-index
                ...
                for TIS_position in all_TIS_positions_gene:
                    if exon_pos[0] <= TIS_position <= exon_pos[1]:
                        ...
                        all_TISs.append(
                            exons_length + TIS_position - exon_pos[0]
                        )  # TIS: 0-index

                exons_length += (
                    exon_pos[1] - exon_pos[0] + 1
                )  # Total length of exons in mRNA
                exon_seq_list.append(
                    chromosome_seq[exon_pos[0] - 1 : exon_pos[1]]
                )  # The total exon sequence
            ...
            (
                ...
                genomic_ATG_coord_correct,
                genomic_ATG_coord_false,
            ) = self.quality_check_extracted_sequences(
                TIS,
                CDS_end,
                exon_seq,
                cds_ends_correct,
            ...
            if TIS == 0:
                exon_seq = (
                    chromosome_seq[(CDS_start - 1) - 300 : CDS_start - 1] + exon_seq
                )
                TIS = 300
                add_seq = 300
                all_TISs = [
                    TIS + 300 for TIS in all_TISs
                ]  # Also adjust all possible TIS
                len_5_UTR = None
            else:
                len_5_UTR = TIS - 1
                add_seq = 0
                ...
                if TIS < 300:
                    add_seq = 300 - TIS
                    exon_seq = (
                        chromosome_seq[(exon_start - 1) - add_seq : exon_start - 1]
                        + exon_seq
                    )
                    TIS = TIS + add_seq
                    all_TISs = [
                        TIS + add_seq for TIS in all_TISs
                    ]  # Also adjust all possible TIS
        ...
        if annotations_dict[transcript_id]["Strand"] == "-":
            ...
            UTR_5_seq = chromosome_seq[
                sorted_exons[-1][-1] : sorted_exons[-1][-1] + 300
            ]

            for exon_pos in sorted_exons:
                exon_seq_list.append(
                    chromosome_seq[exon_pos[0] - 1 : exon_pos[1]]
                )  # The total exon sequence seen from the template strand
            ...
            sorted_exons = sorted(
                annotations_dict[transcript_id]["exon_pos"],
                key=lambda x: x[0],
                reverse=True,
            )
            sorted_CDS = sorted(
                annotations_dict[transcript_id]["CDS_pos"],
                key=lambda x: x[0],
                reverse=True,
            )
            ...
            for exon_pos in sorted_exons:
                ...
                for TIS_position in all_TIS_positions_gene:
                    if exon_pos[0] <= TIS_position <= exon_pos[1]:
                        ...
                        all_TISs.append(
                            exons_length + exon_pos[1] - TIS_position
                        )  # TIS: 0-index
        ...
        if write_transcript_annotation:
            ...
            if not result:
                ...
                if exon_seq[TIS : TIS + 3] == "ATG":
                    TIS_information = pd.concat(
                        [
                            TIS_information,
                            pd.DataFrame(
                                {
                                    "Sequence": exon_seq.upper(),  # Extracted sequence
                                    "ATG_position": TIS,  # Position of ATG to predict on
                                    "TIS": 1,  # TRUE
                                    "codon_type": "TIS",  # "TIS"
                                    "stop_codon": CDS_end
                                    + add_seq,  # Last position of protein coding region
                                    "species": self.species,  # species
                                    ...
                                    "ATG_genomic_position": [
                                        [genomic_ATG_coord, genomic_ATG_coord + 3]
                                    ],  # The TIS position (from genomic data)
                                    "gene": [
                                        annotations_dict[transcript_id]["gene"]
                                    ],  # gene name
                                    "gene_coordinates": [
                    ...
                    non_TIS_information = self.extract_non_TIS_samples(
    ...
    def process_and_extract_sequences(self):
        ...
        try:
            ...
            TIS_information = pd.DataFrame(
                columns=[
                    "Sequence",  # extracted mature mRNA
            ...
            )  # genomic coordinates of transcript that sequence originates from

            # Create empty DataFrame with column names for negative samples (non-TIS ATGs)
            non_TIS_information = pd.DataFrame(
                columns=[
                    "Sequence",  # extracted mature mRNA window
                    "ATG_position",  # Position of ATG to predict on
                    "TIS",  # FALSE (0)
            ...
            with gzip.open(
                f"../../data/data_raw/genomic_fna/{self.species}_genomic.fna.gz", "rt"
            ) as seq_file:
                        ...
                        if chromosome_seq_initialized:
                            ...
                            if extract_info:
                                ...
                                for (
                                    transcript_id,
                                    transcript_info,
                                ) in annotations_dict.items():
                                    if (
                                        "chrom" in transcript_info
                                        and transcript_info["chrom"] == chrom_tag
                                    ):
                                ...
                                for i in range(len(transcripts_in_chromosome)):
                                    ...
                                    (
                                        ...
                                        genomic_ATG_coord_correct,
                                        genomic_ATG_coord_false,
                                    ) = self.extract_strand_information(
                                        annotations_dict=annotations_dict,
                ...
                print(
                    self.species,
                    "Percentage correct stop codons: ",
                    str(cds_ends_correct / (cds_ends_correct + cds_ends_wrong) * 100),
                    " (",
                    str(cds_ends_correct),
                    "/",
                    str(cds_ends_correct + cds_ends_wrong),
                    ")",
                    flush=True,
                )
                ...
                print(
                    self.species,
                    "Percentage correct codon sequences (% 3 = 0): ",
                    str(
                        multiple_3_correct
                        / (multiple_3_correct + multiple_3_wrong)
                        * 100
                    ),
                    " (",
                    str(multiple_3_correct),
                    "/",
                    str(multiple_3_correct + multiple_3_wrong),
                    ")",
                    flush=True,
                )
                ...
                print(
                    self.species,
                    "Percentage of correct genomic placements (ATG start codons placed at genomic TIS coordinates):",
                    str(
                        genomic_ATG_coord_correct
                        / (genomic_ATG_coord_correct + genomic_ATG_coord_false)
                        * 100
                    ),
                    " (",
                    str(genomic_ATG_coord_correct),
                    "/",
                    str(genomic_ATG_coord_correct + genomic_ATG_coord_false),
                    ")",
                    flush=True,
                )
                ...
                print(
                    self.species,
                    "Transcripts without an in-frame stop codon in CDS: ",
                    str(
                        100
                        - (stop_codon_in_cds / (cds_ends_correct + cds_ends_wrong))
                        * 100
                    ),
                    "%",
                    flush=True,
                ...
                TIS_information["Seq_number"] = [
                    "TIS_seq_{}".format(i) for i in range(1, len(TIS_information) + 1)
                ]

                # For non_TIS_information DataFrame
                non_TIS_information["Seq_number"] = [
                    "mRNA_non_TIS_seq_{}".format(i)
                    for i in range(1, len(non_TIS_information) + 1)
                ]

                # Save True-label coding dataset to CSV
                TIS_information.to_csv(
                    f"../../data/data_model_preparation/datasets/TIS/mRNA_positive_{self.species}.csv.gz",
                    index=False,
                    compression="gzip",
                )

                # Save False-label coding dataset to CSV
                non_TIS_information.to_csv(
                    f"../../data/data_model_preparation/datasets/non_TIS/mRNA/mRNA_negative_{self.species}.csv.gz",
                    index=False,
                    compression="gzip",
                )
            ...
            print(
                f"Error: The genomic fasta file for {self.species} was not found.",
                flush=True,
            )

        except PermissionError:
            print(
                f"Error: Permission denied to access the {self.species} genomic fasta file.",
                flush=True,
            )

        except Exception as error:
            print(
                f"An unexpected error occurred while processing the species {self.species} genomic fasta file: {error}",
                flush=True,
            )
...
def process_species(species, group):
    ...
    extractor = ExtractSequences(
        species, group, nts_upstream_extract=500, nts_downstream_extract=500
    )
...
def main():
    ...
    df_species = pd.read_csv(
        "../../data/data_raw/species_information/species_groups.csv"
    )
