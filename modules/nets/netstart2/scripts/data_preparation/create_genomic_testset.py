...
def get_test_sequences_TIS():
    ...
    with gzip.open(
        f"../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_TIS.csv.gz",
        "rt",
    ) as file:
...
def get_species_names():
    ...
    species_list = [
        filename.split("_genomic.gff.gz")[0] for filename in species_filenames_list
    ]
...
def load_gff_file(species):
    ...
    with gzip.open(
        f"../../data/data_raw/genomic_gff/{species}_genomic.gff.gz", "rt"
    ) as file:
...
def extract_genomic_testset(
    species,
    TIS_test_seqs_dict,
    genomic_testsets_dir,
    extend_gene_end_upstream,
    extend_gene_end_downstream,
):
    """
    ...
    outfile_test_sequences = gzip.open(
        f"../../data/data_evaluation/input_testsets/genomic_testsets/{genomic_testsets_dir}/genomic_testset_{species}.fna.gz",
        "wt",
    )
    ...
    test_sequences_data_df = species_TIS_df[
        species_TIS_df["Seq_number"].isin(test_seqs_list)
    ]
    ...
    for entry_line, sequence in read_fasta(species):
        ...
        landmark_sequences_df = test_sequences_data_df[
            test_sequences_data_df["landmark_id"] == landmark
        ]
        ...
        for index, row in landmark_sequences_df.iterrows():
            ...
            if row.strand == "+":
                ...
                assert (
                    sequence[int(atg_coords[0]) : int(atg_coords[1])].upper() == "ATG"
                ), "ATG not found on correct position!"
            elif row.strand == "-":
                atg_coords = row.ATG_genomic_position[1:-1].split(", ")
                assert (
                    reverse_complement(
                        sequence[int(atg_coords[0]) : int(atg_coords[1])].upper()
                    )
                    == "ATG"
                ), "ATG not found on correct position!"

            # Get all gff-annotations of corresponding transcript ID (mRNA, exon, CDS-annotations)
            mrna_annotations_df = species_gff_df[
                species_gff_df[8].str.contains(
                    "rna-" + row["transcript_id"] + ";", na=False
                )
            ]
            gene_annotations_df = species_gff_df[
                species_gff_df[8].str.contains(
                    f"ID=gene-{row['gene']};", regex=False, na=False
                )
            ]
            ...
            if first_CDS_row[6] == "+":
                ...
                exon_end = mrna_annotations_df.loc[
                    (mrna_annotations_df[2] == "exon")
                    & (mrna_annotations_df[4] > start_CDS),
                    4,
                ].min()  # End of the last exon before CDS
                first_downstream_intron_position = (
                    exon_end - start_CDS + 1
                )  # if number is 16, then intron starts at position 17
                ...
                if (
                    gene_row[3] <= extend_gene_end_upstream
                    and gene_row[4] <= len(sequence) - extend_gene_end_downstream
                ):
                ...
                # If the gene is placed in the very end of landmark (number of nucleotides downstream gene is less than 300)
                    ...
                    gene_row[4] >= len(sequence) - extend_gene_end_downstream
                    and gene_row[3] >= extend_gene_end_upstream
                ):
                    ...
                    print(
                        "Identified on + strand: too short 5' and 3' end of landmark."
                    )
                    gene_seq = sequence
                    TIS_pos = start_CDS - 1
                    flanking_seq_missing_5 = "True"
                ...
                # When possible to extract full gene sequence + 300 nucleotides both upstream and downstream
                    ...
                    gene_seq = sequence[
                        gene_row[3] - 1 - extend_gene_end_upstream : gene_row[4]
                        + extend_gene_end_downstream
                    ]
                    TIS_pos = start_CDS - gene_row[3] + extend_gene_end_upstream
            ...
            # If transcript annotations are on complement strand
                ...
                exon_end = mrna_annotations_df.loc[
                    (mrna_annotations_df[2] == "exon")
                    & (mrna_annotations_df[3] < start_CDS),
                    3,
                ].max()  # End of the last exon before CDS
                first_downstream_intron_position = (
                    start_CDS - exon_end + 1
                )  # if number is 8, then intron starts at position 9

                ##extract gene sequence + additional sequence chunk upstream and downstream##
                # If the gene is placed in the very beginning of landmark (number of nucleotides upstream gene is less than 300)
                if (
                    gene_row[3] <= extend_gene_end_downstream
                    and len(sequence) - int(gene_row[4]) >= extend_gene_end_upstream
                ):
                    print("Identified on - strand: too short 3' end of landmark.")
                    gene_seq = reverse_complement(
                        sequence[0 : gene_row[4] + extend_gene_end_upstream]
                    )
                ...
                # If the gene is placed in the very end of landmark (number of nucleotides downstream gene is less than 300)
                    ...
                    len(sequence) - int(gene_row[4]) <= extend_gene_end_upstream
                    and gene_row[3] >= extend_gene_end_downstream
                ):
                    print("Identified on - strand: too short 5' end of landmark.")
                    gene_seq = reverse_complement(
                        sequence[gene_row[3] - 1 - extend_gene_end_downstream :]
                    )
                ...
                # When possible to extract full gene sequence + 300 nucleotides both upstream and downstream
                    ...
                    gene_seq = reverse_complement(
                        sequence[
                            gene_row[3] - 1 - extend_gene_end_downstream : gene_row[4]
                            + extend_gene_end_upstream
                        ]
                    )
                    TIS_pos = gene_row[4] - start_CDS + extend_gene_end_upstream
            ...
            assert extend_gene_end_upstream == 2000, (
                "Upstream region extracted improperly."
            )

            # Ensure that TIS positions are correct
            assert gene_seq[TIS_pos : TIS_pos + 3].upper() == "ATG", print(
                gene_seq, TIS_pos, gene_annotations_df, mrna_annotations_df
            )

            gene_name = row["gene"].replace(" ", "")

            # Write entry line and gene sequence to outfile
            outfile_test_sequences.write(
                ">"
                + gene_name
                + "|species="
                + species
                ...
                + "|annotation_source="
                + annotation_source
                + "|first_downstream_intron_start="
                + str(first_downstream_intron_position)
                + "|5_flanking_sequence_missing="
                + flanking_seq_missing_5
                + "|3_flanking_sequence_missing="
                + flanking_seq_missing_3
                + "|TSS_annotated="
                + TSS_annotated
                + "\n"
            )
...
def remove_duplicates(species, genomic_testsets_dir):
    ...
    with gzip.open(
        f"../../data/data_evaluation/input_testsets/genomic_testsets/{genomic_testsets_dir}/genomic_testset_{species}.fasta.gz",
        "wt",
    ) as output_handle:
...
def extract_genomic_testset_cleaned(
    genomic_testsets_dir, extend_gene_end_upstream, extend_gene_end_downstream
):
