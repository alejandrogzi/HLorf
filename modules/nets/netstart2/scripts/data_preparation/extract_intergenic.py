...
class ExtractIntergenicSequences:
    ...
    def get_intragenic_coords(self):
        ...
        try:
                ...
                for key, value in annotations_dict.items():
                    ...
                    if strand == "+":
                        ...
                        if landmark not in intragenic_coords_landmark.keys():
                            ...
                            intragenic_coords_landmark[landmark].append(
                                gene_coords
                            )  # append gene coordinates
                        else:
                            intragenic_coords_landmark[landmark].append(
                                gene_coords
                            )  # append gene coordinates
            ...
            print(
                f"Error: Permission denied to access the genomic fasta file for {self.species}."
            )
            ...
            print(
                f"An unexpected error occurred while processing the genomic fasta file for {self.species}: {error}"
            )
    ...
    def collect_intergenic_samples(
        self,
        ...
        try:
            intragenic_regions = self.merge_intervals(
                intragenic_regions_dict[landmark_tag]
            )
            ...
            if genes_landmark > 1:
                ...
                for i in range(genes_landmark - 1):
                    ...
                    intergenic_seq = chromosome_seq[
                        intergenic_region_start - 1 : intergenic_region_stop - 1
                    ].upper()

                    # Find positions with "ATG" (0-indexed)
                    atg_indices = [
                        i
                        for i in range(len(intergenic_seq))
                        if intergenic_seq.startswith("ATG", i)
                    ]
                    ...
                    for atg in atg_indices:
                        ...
                        if (
                            atg > self.nts_upstream_extract
                            and len(intergenic_seq) - (atg + 3)
                            ...
                            intergenic_sample = intergenic_seq[
                                atg - self.nts_upstream_extract : atg
                                + 3
                                + self.nts_downstream_extract
                            ]
                            ...
                            assert (
                                chromosome_seq[
                                    intergenic_region_start
                                    - 1
                                    + atg : intergenic_region_start - 1 + atg + 3
                                ].upper()
                                == "ATG"
                            ), "Genomic coordinates not extractd properly."

                            genomic_ATG_coords = [
                                intergenic_region_start - 1 + atg,
                                intergenic_region_start - 1 + atg + 3,
                            ]
                            ...
                            if not result:
                                ...
                                intergenic_information = pd.concat(
                                    [
                                        intergenic_information,
                                        pd.DataFrame(
                                            {
                                                "Sequence": [
                                                    intergenic_sample.upper()
    ...
    def extract_intergenic_samples(self):
        ...
        chromosome_seq = ""
        counter_samples = 0
        intergenic_information = pd.DataFrame(
            columns=[
        ...
        try:
                ...
                for line in seq_file:
                    ...
                    if line.startswith(">"):
                        ...
                        if chromosome_seq_initialized:
                            ...
                            if (
                                extract_info
                                and landmark_tag in intragenic_regions_dict.keys()
                            ):
                                # add non-coding sequences from one chromosome
                                intergenic_information, counter_samples = (
                                    self.collect_intergenic_samples(
                                        intergenic_information=intergenic_information,
            ...
            if len(intergenic_information) > positive_samples:
                intergenic_information = intergenic_information.sample(
            ...
            intergenic_information["Seq_number"] = [
                "intergenic_seq{}".format(i)
                for i in range(1, len(intergenic_information) + 1)
            ]

            # Save non-coding data to CSV
            intergenic_information.to_csv(
                f"../../data/data_model_preparation/datasets/non_TIS/intergenic/intergenic_data_{self.species}.csv.gz",
                index=False,
                compression="gzip",
            )
...
def process_species(species, group):
    print("\n\n")
    print(species)
    extractor = ExtractIntergenicSequences(
