...
class GFFProcesser:
            ...
            for line in info_file:
                ...
                if len(col_info) >= 3:
                    ...
                    if col_info[1] in [
                        ...
                        if (
                            col_info[2] == "gene"
                            and "gene_biotype=protein_coding" in line
                            and "exception=trans-splicing" not in line
                        ):
    ...
    def process_gff_file(self):
                ...
                if len(col_info) >= 3:
                    ...
                    if (
                        ...
                        if col_info[2] == "mRNA":
                            ...
                            if (
                                initial_mRNA_found
                                and transcript_id in self.transcripts_info_dict.keys()
                            ):
                                ...
                                if match_gene:
                                    ...
                                    if gene_id in self.genes_info_dict.keys():
                                        ...
                                        self.transcripts_info_dict[transcript_id] = {
                                            ...
                                            "gene_coordinates": genes_info_dict[
                                                gene_id
                                            ],
                                        }  # Gene coordinates
                        ...
                        if col_info[2] in ["exon", "CDS"]:
                            ...
                            match_annotations = re.search(
                                r"Parent=rna-([^;]+)", attr_col
                            )
                            if match_annotations:
                                ...
                                if match_annotations.group(1) == transcript_id:
                                    if (
                                        transcript_id
                                        in self.transcripts_info_dict.keys()
                                    ):
                                            ...
                                            if match_prot_name:
                                                if not prot_name_found:
                                                    ...
                                                    assert (
                                                        ...
                                                        "Transcript annotations corresponds to more than one protein."
                                                    )
        ...
        print(
            ...
            len(self.transcripts_info_dict.keys()),
            flush=True,
        )
        ...
        for transcript_id in self.transcripts_info_dict:
            ...
            for (i, exon_coordinates1), (j, exon_coordinates2) in combinations(
                enumerate(self.transcripts_info_dict[transcript_id]["exon_pos"]), 2
            ):
                assert exon_coordinates1 != exon_coordinates2, (
                    "Duplicate exon annotation found."
                )

            # Check that there are no duplicate CDS position sets
            for (i, CDS_coordinates1), (j, CDS_coordinates2) in combinations(
                enumerate(self.transcripts_info_dict[transcript_id]["CDS_pos"]), 2
            ):
                assert CDS_coordinates1 != CDS_coordinates2, (
                    "Duplicate CDS annotation found"
                )
    ...
    def check_annotations(self):
        ...
        for key, value in annotations_dict.items():
            try:
                ...
                assert len(cds_positions) == len(unique_cds_positions), (
                    "A CDS sequence occurs occurs twice for a transcript."
                )
                ...
                assert len(cds_start) == len(set(cds_start)), (
                    "Something is wrong with a start coordinate of CDS."
                )
                ...
                assert len(cds_end) == len(set(cds_end)), (
                    "Something is wrong with a stop coordinate of CDS."
                )
