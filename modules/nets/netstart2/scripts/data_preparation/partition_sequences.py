...
def clean_and_partition_sequences(sequence_type):
    ...
    if sequence_type not in [
        ...
        "intergenic",
        "intron",
        "mRNA_non_TIS_upstream",
        "mRNA_non_TIS_downstream",
    ]:
        raise ValueError(
            "Invalid sequence_type. Must be one of: 'TIS', 'intergenic', 'intron', 'mRNA_non_TIS_upstream', 'mRNA_non_TIS_downstream'."
        )

    # Load species information and store
    df_species_details = pd.read_csv(
    ...
    for species in species_list:
        ...
        df_species = pd.read_csv(
            "../../data/data_model_preparation/datasets/"
            + subpath
            + species
            + ".csv.gz",
            compression="gzip",
        ...
        with open(
            ...
            for row in df_all_species.itertuples():
                if row.codon_type.startswith(sequence_position):
                    assert (
                        row.Sequence[row.ATG_position : row.ATG_position + 3] == "ATG"
                    )

                    seq = row.Sequence[
                        row.ATG_position - 300 : row.ATG_position + 3 + 300
                    ]
    ...
    # Run graphpart
    graphpart_command = (
        f"graphpart mmseqs2 -nu --fasta-file ../../data/data_model_preparation/datasets_fasta/"
        + sequence_type
        + "_sequences.fasta --threshold 0.5 --out-file ../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_"
        + sequence_type
        + ".csv --labels-name label --partitions 5"
    )
    ...
    # Gzip fastafile
    gzip_command = (
        f"gzip ../../data/data_model_preparation/datasets_fasta/"
        + sequence_type
        + "_sequences.fasta"
    )
