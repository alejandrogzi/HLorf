...
def extract_partitions_data(partition_writers, seq_type):
    ...
    with gzip.open(
        "../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_"
        + seq_type
        + ".csv.gz",
        "rt",
    ) as assignment_file:
    ...
    with gzip.open(
        "../../data/data_model_preparation/datasets_fasta/"
        + seq_type
        + "_sequences.fasta.gz",
        "rt",
    ) as seq_file:
...
def convert_fasta_to_csv(
    ...
    gzipped_csv_file_name = (
        "../../data/data_model/datasets/data_partition_" + partition_number + ".csv.gz"
    )
    ...
    with gzip.open(gzipped_csv_file_name, "wt", newline="") as csv_file:
        ...
        with gzip.open(
            "../../data/data_model_preparation/datasets_partitioned/data_partition_"
            + partition_number
            + ".fasta.gz",
            "rt",
        ) as fasta_file:
            ...
            for line in fasta_file:
                ...
                if line.startswith(">"):
                    ...
                    if "type=TIS" in line:
                        ...
                        species_name = (
                            species_name[0].capitalize()
                            + " "
                            + " ".join(subname.lower() for subname in species_name[1:])
                        )
                    ...
                    # Intergenic sequences
                        ...
                        seq_number = (
                            "intergenic_seq_"
                            + seq_id.split("intergenic_seq")[1].split("_")[0]
                        )
                        species_name = line.split("|")[0].split("seq")[1].split("_")[1:]
                        species_name = (
                            species_name[0].capitalize()
                ...
                # Sequence line
                    ...
                    assert sequence[300:303] == "ATG", (
                        "TIS ATG is not at position 301-303"
                    )
    ...
    print(
        "Data has been converted and saved to",
        gzipped_csv_file_name,
        sep=" ",
        flush=True,
    )
...
def check_sequences_duplicates(partition_number):
    ...
    print(
        "Shape before removing duplicates across sequence types: ",
        df_partition.shape,
        flush=True,
    )
    ...
    print(
        "Shape after removing ALL duplicates across sequence types: ",
        df_partition.shape,
        flush=True,
    )

    df_partition_correct = pd.concat([df_TIS, df_partition])
    df_partition_correct = df_partition_correct.drop_duplicates(
        subset=["Sequence"], keep="first"
    )

    print(
        "Shape after removing false duplicates across sequence types (keeping TIS): ",
        df_partition_correct.shape,
        flush=True,
    )

    df_partition_correct.to_csv(
        "../../data/data_model/datasets/data_partition_" + partition_number + ".csv.gz",
        compression="gzip",
        index=False,
    )
...
partition_writers = {
    key: gzip.open(value, "wt") for key, value in partition_files.items()
}
...
partition_writers = extract_partitions_data(
    partition_writers, "mRNA_non_TIS_downstream"
)
...
annotation_source_seqs_TIS, annotation_source_seqs_mRNA_non_TIS, utr_5_length = (
...
convert_fasta_to_csv(
    "1", annotation_source_seqs_TIS, annotation_source_seqs_mRNA_non_TIS, utr_5_length
)
check_sequences_duplicates("1")
print("Converting partition 2 to CSV", flush=True)
convert_fasta_to_csv(
    "2", annotation_source_seqs_TIS, annotation_source_seqs_mRNA_non_TIS, utr_5_length
)
check_sequences_duplicates("2")
print("Converting partition 3 to CSV", flush=True)
convert_fasta_to_csv(
    "3", annotation_source_seqs_TIS, annotation_source_seqs_mRNA_non_TIS, utr_5_length
)
check_sequences_duplicates("3")
print("Converting partition 4 to CSV", flush=True)
convert_fasta_to_csv(
    "4", annotation_source_seqs_TIS, annotation_source_seqs_mRNA_non_TIS, utr_5_length
)
check_sequences_duplicates("4")
print("Converting partition 5 to CSV", flush=True)
convert_fasta_to_csv(
    "5", annotation_source_seqs_TIS, annotation_source_seqs_mRNA_non_TIS, utr_5_length
)
