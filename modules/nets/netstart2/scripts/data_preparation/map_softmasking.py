...
def extract_transcript_sequence(
    annotations_dict, transcript_id, chromosome_seq, extend_sequence
):
    """
    ...
    if annotations_dict[transcript_id]["Strand"] == "+":
        ...
        if extend_sequence:
            if sorted_exons[0][0] < 500:
                exon_seq = (
                    chromosome_seq[: sorted_exons[0][0] - 1]
                    + exon_seq
                    + chromosome_seq[sorted_exons[-1][-1] : sorted_exons[-1][-1] + 500]
                )
            else:
                exon_seq = (
                    chromosome_seq[sorted_exons[0][0] - 501 : sorted_exons[0][0] - 1]
                    + exon_seq
                    + chromosome_seq[sorted_exons[-1][-1] : sorted_exons[-1][-1] + 500]
                )
            ...
            if TIS == 0:
                exon_seq = (
                    chromosome_seq[(CDS_start - 1) - 180 : CDS_start - 1] + exon_seq
                )
                TIS = 180
                TSS_annotated = "False"
                if CDS_start < 180:
    ...
    if annotations_dict[transcript_id]["Strand"] == "-":
        ...
        if not extend_sequence:
            ...
            UTR_5_seq = chromosome_seq[
                sorted_exons[-1][-1] : sorted_exons[-1][-1] + 180
            ]
        ...
        if extend_sequence:
            # Extract 180 nucleotides upstream TIS (use for cases where 5' UTR length is not annotated)
            upstream_UTR_5_seq = chromosome_seq[
                sorted_exons[-1][-1] : sorted_exons[-1][-1] + 500
            ]
            downstream_UTR_3_seq = chromosome_seq[
                sorted_exons[0][0] - 500 : sorted_exons[0][0]
            ]

            # Get complement strand sequence by taking reverse complement of sequence
            extended_transcript_seq = reverse_complement(
                downstream_UTR_3_seq + exon_seq + upstream_UTR_5_seq
            )
        ...
        sorted_CDS = sorted(
            annotations_dict[transcript_id]["CDS_pos"], key=lambda x: x[0], reverse=True
        )
...
species_filenames_list = os.listdir(
    "../../data/data_evaluation/input_testsets/mRNA_testsets"
)
species_list = [
    filename.split(".fasta.gz")[0].split("input_testset_")[1]
    for filename in species_filenames_list
]
...
for species in species_list:
    ...
    with open(
        f"../../data/data_model_preparation/transcripts_info/{species}_dict.json", "r"
    ) as dict_file:
        annotations_dict = json.load(dict_file)

    # open outfile to write softmasked test sequences to
    with gzip.open(
        f"../../data/data_evaluation/input_testsets/mRNA_testsets_processed/input_testset_{species}_softmasked.fasta.gz",
        "wt",
    ) as outfile:
        test_sequences = {}
        ...
        with gzip.open(
            f"../../data/data_evaluation/input_testsets/mRNA_testsets/input_testset_{species}.fasta.gz",
            "rt",
        ) as file:
            for line in file:
                if line.startswith(">"):
        ...
        intron_df = pd.read_csv(
            f"../../data/data_model_preparation/datasets/non_TIS/introns/introns_{species}.csv.gz",
            compression="gzip",
        )
        ...
        intergenic_df = pd.read_csv(
            f"../../data/data_model_preparation/datasets/non_TIS/intergenic/intergenic_data_{species}.csv.gz",
            compression="gzip",
        )
        intergenic_test_df = intergenic_df[
            intergenic_df["Seq_number"].isin(test_sequences.keys())
        ]

        # Get full transcript testdata information (TIS-labelled sequences)
        TIS_df = pd.read_csv(
            f"../../data/data_model_preparation/datasets/TIS/mRNA_positive_{species}.csv.gz",
            compression="gzip",
        )
        ...
        mRNA_non_TIS_df = pd.read_csv(
            f"../../data/data_model_preparation/datasets/non_TIS/mRNA/mRNA_negative_{species}.csv.gz",
            compression="gzip",
        )
        mRNA_non_TIS_test_df = mRNA_non_TIS_df[
            mRNA_non_TIS_df["Seq_number"].isin(test_sequences.keys())
        ]

        # Loop through genome fasta file (softmasked)
        with gzip.open(
            f"../../data/data_raw/genomic_fna/{species}_genomic.fna.gz", "rt"
        ) as file:
            # Initialize
            initiated = False
            ...
            for line in file:
                ...
                if line.startswith(">"):
                    ...
                    if initiated:
                        ...
                        if TIS_landmark_seqs_df.shape[0] != 0:
                            ...
                            for i, row in TIS_landmark_seqs_df.iterrows():
                                transcript_seq_softmasked, TIS, TSS_annotated = (
                                ...
                                if (
                                    len(transcript_seq_softmasked) < 30000
                                    and transcript_seq_softmasked != ""
                                ):
                                    # Make sure transcript sequence as been correctly extracted
                                    assert (
                                        transcript_seq_softmasked.upper()
                                        in row["Sequence"]
                                        or row["Sequence"]
                                        in transcript_seq_softmasked.upper()
                                    ), "Neither sequence is contained within the other."
                                    assert (
                                        transcript_seq_softmasked[TIS : TIS + 3].upper()
                                        == "ATG"
                                    ), print(
                                        TIS, transcript_seq_softmasked, TSS_annotated
                                    )  # "Transcript sequence not properly extracted!"
                        ...
                        if mRNA_non_TIS_landmark_seqs_df.shape[0] != 0:
                            ...
                            for i, row in mRNA_non_TIS_landmark_seqs_df.iterrows():
                                ...
                                (
                                    transcript_seq_softmasked_extended,
                                    TIS,
                                    TSS_annotated,
                                ) = extract_transcript_sequence(
                                    annotations_dict,
                                ...
                                test_seq_start = (
                                    transcript_seq_softmasked_extended.upper().find(
                                        test_sequence_uppercase
                                    )
                                )
                                test_seq_end = test_seq_start + len(
                                    test_sequence_uppercase
                                )

                                # Get softmasked test sequence
                                test_sequence_softmasked = (
                                    transcript_seq_softmasked_extended[
                                        test_seq_start:test_seq_end
                                    ]
                                )

                                assert (
                                    test_sequence_uppercase
                                    == test_sequence_softmasked.upper()
                                )
                                assert (
                                    test_sequence_softmasked[500:503].upper() == "ATG"
                                )

                                # Write soft-masked version of sequence to outfile
                                outfile.write(
                                    test_sequences[row["Seq_number"]].strip()
                                    + "|TSS_annotated="
                                    + TSS_annotated
                                    + "\n"
                                )
                        ...
                        if intergenic_landmark_seqs_df.shape[0] != 0:
                            ...
                            for i, row in intergenic_landmark_seqs_df.iterrows():
                                ...
                                ATG_coords = (
                                    row["ATG_genomic_position"]
                                    .split("[")[1]
                                    .split("]")[0]
                                    .split(", ")
                                )
                                softmasked_seq = landmark_seq[
                                    int(ATG_coords[0]) - 500 : int(ATG_coords[1]) + 500
                                ]

                                assert (
                                    landmark_seq[
                                        int(ATG_coords[0]) : int(ATG_coords[1])
                                    ].upper()
                                    == "ATG"
                                )
                        ...
                        if intron_landmark_seqs_df.shape[0] != 0:
                            ...
                            for i, row in intron_landmark_seqs_df.iterrows():
                                ...
                                if row["strand"] == "+":
                                    ...
                                    softmasked_seq = landmark_seq[
                                        int(ATG_coords[0]) - 500 : int(ATG_coords[1])
                                        + 500
                                    ]
                                ...
                                # complement strand
                                    ...
                                    # get full softmasked test sequence
                                    softmasked_seq = reverse_complement(
                                        landmark_seq[
                                            int(ATG_coords[0]) - 500 : int(
                                                ATG_coords[1]
                                            )
                                            + 500
                                        ]
                    ...
                    intergenic_landmark_seqs_df = intergenic_test_df[
                        intergenic_test_df["landmark_id"] == landmark_tag
                    ]
                    intron_landmark_seqs_df = intron_test_df[
                        intron_test_df["landmark_id"] == landmark_tag
                    ]
                    TIS_landmark_seqs_df = TIS_test_df[
                        TIS_test_df["landmark_id"] == landmark_tag
                    ]
                    mRNA_non_TIS_landmark_seqs_df = mRNA_non_TIS_test_df[
                        mRNA_non_TIS_test_df["landmark_id"] == landmark_tag
                    ]
