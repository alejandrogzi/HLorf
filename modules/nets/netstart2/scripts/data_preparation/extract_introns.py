...
class ExtractIntronSequences:
    ...
    def count_positives(self):
        ...
        try:
            with gzip.open(
                "../../data/data_model_preparation/datasets/TIS/mRNA_positive_"
                + self.species
                + ".csv.gz",
                "rb",
            ) as gzipped_file:
                ...
                positive_samples = (
                    sum(1 for line in gzipped_file) - 1
                )  # -1 due to information row
    ...
    def extract_strand_information(
        self, annotations_dict, transcript_id, chromosome_seq, intron_information
    ):
        """
        ...
        try:
            ...
            if annotations_dict[transcript_id]["Strand"] == "+":
                ...
                for i in range(len(sorted_exons) - 1):
                    introns_coords.append(
                        [sorted_exons[i][1] - 1, sorted_exons[i + 1][0]]
                    )
                ...
                for coords in introns_coords:
                    ...
                    if coords[1] + self.nts_downstream_extract < len(chromosome_seq):
                        if coords[0] - self.nts_upstream_extract > 0:
                            ...
                            atg_indices = [
                                i
                                for i in range(len(intron_seq))
                                if intron_seq.upper().startswith("ATG", i)
                            ]

                            random.shuffle(atg_indices)

                            # Extract "framed" intron sequence
                            intron_seq_framed = chromosome_seq[
                                coords[0] - self.nts_upstream_extract : coords[1]
                            ...
                            for atg in atg_indices:
                                ...
                                assert (
                                    len(
                                        intron_seq_framed[
                                            atg_adjusted
                                            - self.nts_upstream_extract : atg_adjusted
                                            + 3
                                            + self.nts_downstream_extract
                                        ]
                                    )
                                    == self.nts_upstream_extract
                                    + 3
                                    + self.nts_downstream_extract
                                ), "Intron sequence not properly extracted."

                                # Extract intron subsequence
                                intron_subseq = intron_seq_framed[
                                    atg_adjusted
                                    - self.nts_upstream_extract : atg_adjusted
                                    + 3
                                    + self.nts_downstream_extract
                                ...
                                assert (
                                    intron_subseq[
                                        self.nts_upstream_extract : self.nts_upstream_extract
                                        + 3
                                    ].upper()
                                    == "ATG"
                                ...
                                if not result:
                                    ...
                                    assert (
                                        chromosome_seq[
                                            genomic_ATG_coords[0] : genomic_ATG_coords[
                                                1
                                            ]
                                        ].upper()
                                        == "ATG"
                                            ...
                                            pd.DataFrame(
                                                {
                                                    "Sequence": [
                                                        intron_subseq.upper()
                                                    ],  # The extracted non-coding sequence
                                                    ...
                                                    "ATG_genomic_position": [
                                                        [
                                                            genomic_ATG_coords[0],
                                                            genomic_ATG_coords[1],
                                                        ]
                                                    ],  # Genomic position of ATG
                                                    "gene": [
                                                        annotations_dict[transcript_id][
                                                            "gene"
                                                        ]
                                                    ],  # the name of the gene that intron arises from
                                                    "gene_coordinates": [
                                                        annotations_dict[transcript_id][
                                                            "gene_coordinates"
                                                        ]
                ...
                for coords in introns_coords:
                    ...
                    if coords[1] + self.nts_downstream_extract < len(chromosome_seq):
                        if coords[0] - self.nts_upstream_extract > 0:
                            ...
                            intron_seq = self.reverse_complement(
                                chromosome_seq[coords[0] : coords[1]]
                            )
                            intron_seq_framed = self.reverse_complement(
                                chromosome_seq[
                                    coords[0] - self.nts_upstream_extract : coords[1]
                                    + self.nts_upstream_extract
                                ]
            ...
            if (
                len(
                    intron_seq_framed[
                        atg - self.nts_upstream_extract : atg
                        + 3
                        + self.nts_downstream_extract
                    ]
                )
    ...
    def process_and_extract_sequences(self):
        ...
        try:
            ...
            intron_information = pd.DataFrame(
                columns=[
                    "Sequence",  # The extracted non-coding sequence
            ...
            with gzip.open(
                ...
                if extract_info and not extracted_all:
                    ...
                    # Loop over every transcript seperately to extract information individually
                    for i in range(len(transcripts_in_chromosome)):
                        intron_information = self.extract_strand_information(
                            annotations_dict=annotations_dict,
                            transcript_id=transcripts_in_chromosome[i],
                            chromosome_seq=chromosome_seq,
                            intron_information=intron_information,
            ...
            if len(intron_information) > positive_samples:
                intron_information = intron_information.sample(
                    n=positive_samples, random_state=42
                )

            # For TIS_information DataFrame
            intron_information["Seq_number"] = [
                "intron_seq_{}".format(i) for i in range(1, len(intron_information) + 1)
            ]

            # Save ATG-labelled intron dataset to CSV
            intron_information.to_csv(
                f"../../data/data_model_preparation/datasets/non_TIS/introns/introns_{self.species}.csv.gz",
                index=False,
                compression="gzip",
            )
...
def process_species(species, group):
    print("\n\n")
    print(species)
    extractor = ExtractIntronSequences(
