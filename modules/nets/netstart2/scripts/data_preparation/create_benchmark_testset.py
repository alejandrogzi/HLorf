...
TIS_testset = df_TIS_partitions[
    df_TIS_partitions["cluster"] == 4.0
]  # clusters are assigned 0, 1, 2, 3, 4; 4 is test cluster
...
with gzip.open(
    f"../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_intergenic.csv.gz",
    "rt",
) as file:
    df_intergenic_partitions = pd.read_csv(file)

intergenic_testset = df_intergenic_partitions[
    df_intergenic_partitions["cluster"] == 4.0
]  # (clusters are assigned 0, 1, 2, 3, 4; 4 is test cluster)
...
# Intron sequences
with gzip.open(
    f"../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_intron.csv.gz",
    "rt",
) as file:
    df_intron_partitions = pd.read_csv(file)

intron_testset = df_intron_partitions[
    df_intron_partitions["cluster"] == 4.0
]  # (clusters are assigned 0, 1, 2, 3, 4; 4 is test cluster)
...
with gzip.open(
    f"../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_mRNA_non_TIS_upstream.csv.gz",
    "rt",
) as file:
    df_mrna_upstream_partitions = pd.read_csv(file)

mrna_upstream_testset = df_mrna_upstream_partitions[
    df_mrna_upstream_partitions["cluster"] == 4.0
]  # (clusters are assigned 0, 1, 2, 3, 4; 4 is test cluster)
...
with gzip.open(
    f"../../data/data_model_preparation/graphpart_assignments/graphpart_assignments_mRNA_non_TIS_downstream.csv.gz",
    "rt",
) as file:
    df_mrna_downstream_partitions = pd.read_csv(file)

mrna_downstream_testset = df_mrna_downstream_partitions[
    df_mrna_downstream_partitions["cluster"] == 4.0
]  # (clusters are assigned 0, 1, 2, 3, 4; 4 is test cluster)
...
for species in species_list:
    ...
    with gzip.open(
        ...
        TIS_test_seqs_df = df_TIS_sequences[
            df_TIS_sequences["Seq_number"].isin(TIS_test_seqs_dict[species])
        ]
    ...
    with gzip.open(
        ...
        mRNA_non_TIS_test_seqs_df = df_mRNA_non_TIS_sequences[
            df_mRNA_non_TIS_sequences["Seq_number"].isin(
                mRNA_non_TIS_test_seqs_dict[species]
            )
        ]
    ...
    with gzip.open(
        ...
        intergenic_test_seqs_df = df_intergenic_sequences[
            df_intergenic_sequences["Seq_number"].isin(
                intergenic_test_seqs_dict[species]
            )
        ]
    ...
    with gzip.open(
        ...
        try:
            intron_test_seqs_df = df_introns_sequences[
                df_introns_sequences["Seq_number"].isin(intron_test_seqs_dict[species])
            ]
