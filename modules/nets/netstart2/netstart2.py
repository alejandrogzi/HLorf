...
MODEL_CONFIG_DIR = BASE_DIR / "src" / "model_config"
TAXONOMY_DIR = MODEL_CONFIG_DIR / "taxonomy"
...
def validate_origin(origin):
    ...
    return (
        False,
        f"Invalid origin. Must be one of the valid species or phyla. Got: {origin}",
    )
    ...
    try:
        with open_func(filename, "rt") as f:
            ...
            if not first_line.startswith(">"):
                return (
                    False,
                    f"File is not in FASTA format: should start with '>'. Got: {first_line}",
                )
...
def load_taxonomy_mappings():
    ...
    with (TAXONOMY_DIR / "species_names.json").open("r") as file:
        species_names_dict = json.load(file)
    with (TAXONOMY_DIR / "species_to_kingdom.json").open("r") as file:
        species_kingdom_dict = json.load(file)
    with (TAXONOMY_DIR / "species_to_phylum.json").open("r") as file:
        species_phylum_dict = json.load(file)
    with (TAXONOMY_DIR / "species_to_class.json").open("r") as file:
        species_class_dict = json.load(file)
    with (TAXONOMY_DIR / "species_to_order.json").open("r") as file:
        species_order_dict = json.load(file)
    with (TAXONOMY_DIR / "species_to_family.json").open("r") as file:
        species_family_dict = json.load(file)
    with (TAXONOMY_DIR / "species_to_genus.json").open("r") as file:
...
def get_formatted_nucleotide_sequence(
    sequence,
    position,
...
def get_formatted_amino_acid_sequence(
    sequence,
    position,
    ...
    if nucleotides_upstream < extract_upstream_aa * 3:
        ...
        sequence_aa += encode_nucleotide_to_amino_acid(
            sequence[position - nucleotides_upstream : position]
        )
    else:
        sequence_aa += encode_nucleotide_to_amino_acid(
            sequence[position - extract_upstream_aa * 3 : position]
        )

    # If needed to extract more nucleotides than there is in sequence downstream
    if nucleotides_downstream < extract_downstream_aa * 3:
        if nucleotides_downstream % 3 != 0:
            nucleotides_downstream = nucleotides_downstream - (
                nucleotides_downstream % 3
            )

        aa_downstream = int(nucleotides_downstream / 3)

        sequence_aa += encode_nucleotide_to_amino_acid(
            sequence[position : position + 3 + nucleotides_downstream]
        )
        pad_positions_aa = int(extract_downstream_aa - aa_downstream)
        sequence_aa += pad_positions_aa * pad_token
    else:
        sequence_aa += encode_nucleotide_to_amino_acid(
            sequence[position : position + 3 + extract_downstream_aa * 3]
        )
...
def extract_datasets(
    input_filename, extract_upstream_aa, extract_downstream_aa, tax_mappings, origin
):
    """
...
def extract_datasets_complement_strand(
    input_filename, extract_upstream_aa, extract_downstream_aa, tax_mappings, origin
):
    """
    Get part of datasets that is the same across trained models in correct format to run NetStart 2.0 on.
            ...
            rows.append(
                {
                    ...
                    "origin": origin,
                    "entry_line": header,
                    "atg_position": position_1_indexed
                    - 1,  # ATG Position relative to full sequence (1-indexed)
                    "stop_codon_position": first_stop_codon_pos_1_indexed,  # Stop Codon Position relative to full sequence (1-indexed) (position of first base in stop codon)
                    "peptide_len": aa_seq_len,
                    "strand": "-",
                }
...
def extract_datasets_nt(input_filename, extract_upstream_nt, extract_downstream_nt):
    """
    ...
    for header, sequence in read_fasta(input_filename):
        ...
        for position in positions:
            ...
            sequence_nt = get_formatted_nucleotide_sequence(
                sequence,
                position,
                nucleotides_downstream,
                extract_upstream_nt,
            ...
            rows.append({"nt_sequences": sequence_nt})

    df_input_nt = pd.DataFrame(rows)
    return df_input_nt


def extract_datasets_nt_complement_strand(
    input_filename, extract_upstream_nt, extract_downstream_nt
):
    """
...
def create_encodings_aa(
    df_input, extract_upstream_aa, extract_downstream_aa, batch_size=64
):
    """
...
class NetstartModel(nn.Module):
    ...
    def __init__(
        ...
        self.pretrained_model_aa = AutoModel.from_pretrained(
            str(PRETRAINED_DIR / f"esm2-8m-finetuned_model_100u_100d_model{model_no}")
        )
...
def ExtractDataAndModel(
    ...
    hyperparam_path = HYPERPARAM_DIR / f"netstart_model{model_no}_hyperparameters.json"
    with hyperparam_path.open("r") as json_file:
        model_config = json.load(json_file)

    # Get processed sequences and required information for model
    df_input_nt = extract_datasets_nt(
        input_filename,
    ...
    if include_rev_comp:
        df_input_nt_rev_comp = extract_datasets_nt_complement_strand(
            input_filename,
            extract_upstream_nt=model_config["nt_upstream"],
            extract_downstream_nt=model_config["nt_downstream"],
        )

        df_input_nt = pd.concat(
            [df_input_nt, df_input_nt_rev_comp], axis=0, ignore_index=True
        )

    # Create nucleotide encodings
    encodings_nt = create_encodings_nt(df_input_nt)

    nt_encodings_len_flattened = len(encodings_nt[0]) * len(
        encodings_nt[0][0]
    )  # 4 * numbers of nt
    ...
    if hasattr(state_dict_result, "missing_keys"):
        ...
        print(
            "PyTorch version does not support structured return from load_state_dict()"
        )
...
def get_predictions(
    ...
    if include_rev_comp:
        ...
        df_input_rev_comp = extract_datasets_complement_strand(
            input_filename,
            extract_upstream_aa=100,
            extract_downstream_aa=100,
            tax_mappings=tax_mappings,
    ...
    if len(df_input) > 0:
        ...
        dataset_invariant = MultiInputDataset(
            encodings_aa,
            df_input["tax_ranks"],
            df_input["entry_line"],
        ...
        test_loader_invariant = DataLoader(
            dataset_invariant, batch_size=batch_size, shuffle=False
        )

        # Get varying data and models
        model1, test_loader1 = ExtractDataAndModel(
            model_no="1",
        ...
        model2, test_loader2 = ExtractDataAndModel(
            model_no="2",
            input_filename=input_filename,
            aa_encodings_len=aa_encodings_len,
            vocab_sizes=vocab_sizes,
        ...
        model3, test_loader3 = ExtractDataAndModel(
            model_no="3",
            input_filename=input_filename,
            aa_encodings_len=aa_encodings_len,
            vocab_sizes=vocab_sizes,
        ...
        model4, test_loader4 = ExtractDataAndModel(
            model_no="4",
            input_filename=input_filename,
            aa_encodings_len=aa_encodings_len,
            vocab_sizes=vocab_sizes,
        ...
        with torch.no_grad():
            for batch_1, batch_2, batch_3, batch_4, batch_invariant in tqdm(
                zip(
                    test_loader1,
                    test_loader2,
                    test_loader3,
                    test_loader4,
                    test_loader_invariant,
                ),
                total=total_batches,
                desc="Predicting",
            ):
                ...
                attention_mask_aa = batch_invariant["aa_encodings"][
                    "attention_mask"
                ].to(device)
                ...
                information_dict["preds"].extend(
                    outputs_averaged.cpu().numpy().flatten()
                )
                information_dict["origin"].extend(batch_invariant["origin"])
                information_dict["atg_position"].extend(batch_invariant["atg_position"])
                information_dict["stop_codon_position"].extend(
                    batch_invariant["stop_codon_position"]
                )
        ...
        information_dict["preds"] = [
            round(float(val), 6) for val in information_dict["preds"]
        ]
        information_dict["atg_position"] = [
            int(val) for val in information_dict["atg_position"]
        ]
        information_dict["stop_codon_position"] = [
            int(val.item()) if not val.item() == -1 else ""
            for val in information_dict["stop_codon_position"]
        ]
        information_dict["peptide_len"] = [
            int(val.item()) if not torch.isnan(val) else float("nan")
            for val in information_dict["peptide_len"]
        ]
        ...
        if output_results == "max_prob":
            ...
            information_df = information_df.loc[
                information_df.groupby("entry_line")["preds"].idxmax()
            ]
        ...
        if gzip_outfile:
            information_df.to_csv(
                output_filename + ".csv.gz", index=False, compression="gzip"
            )
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Predict Eukaryotic Translation Initiation Sites with NetStart 2.0. The model returns the probability of a given ATG being a start codon.\
    ...
    optional.add_argument(
        "--batch_size", type=int, default=64, help="Set batch size (default: 64)."
    )
